    ...
    <div class="flex items-center justify-between border-b border-slate-200 dark:border-gray-800 pb-5">
        <div>
            ...
            <p class="text-xs sm:text-sm text-slate-500 dark:text-gray-400 mt-1">
                Daftarkan akun pengguna baru dan tentukan hak akses peran sistem.
            </p>
        </div>
        ...
        <form action="{{ route('users.store') }}" method="POST" class="space-y-6">
            ...
            <div class="space-y-4 pt-2">
                ...
                <div class="space-y-2">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3" x-data="{ selectedRole: '{{ old('role', 'operator') }}' }">
                        <!-- Role Operator -->
                        <label class="relative flex flex-col p-4 rounded-xl border cursor-pointer transition select-none"
                            ...
                            <p class="text-[11px] text-slate-500 dark:text-gray-400 leading-relaxed">
                                Dapat mengelola Project Campaign, media pool, dan memantau status antrean posting.
                            </p>
                        </label>
                ...
                <div class="space-y-1.5 pt-2">
                    ...
                    <select name="status" 
                        ...
                        <option value="inactive" {{ old('status') === 'inactive' ? 'selected' : '' }}>Nonaktif (Diblokir)</option>
                    </select>
                    @error('status')
                        <p class="text-[11px] text-rose-500">{{ $message }}</p>
                    @enderror
